...
function findNetAttchment(elem)
{
	...
	elem.setAttribute("used",true);
		 ...
		 {   
			 ...
			 var a=strToBool(netElem.getAttribute("used"));
			 
			 if(!a)
			  {
				netElem.setAttribute('ref',ref);
				netElem.setAttribute("used",true);
...
function modifiedRefNetWithStdPart()
{
 ...
 for(var i=0; i<parts.length;i++)
	 if(strToBool(parts[i].firstChild.getAttribute('std'))&& (parts[i].firstChild.getAttribute('modelname')=='GND')){
