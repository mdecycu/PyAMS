...
function posInBody(myPics,x,y)
  {

     function getMousePosition(evt) {
          var CTM = svg.getScreenCTM();
          return {
            x: 10*Math.round((evt.clientX - CTM.e) / (CTM.a*10)),
            y: 10*Math.round((evt.clientY - CTM.f) / (CTM.d*10))
          };
		  
        }
	
    myPics.addEventListener('mousemove', e => {
	x.innerHTML='x='+getMousePosition(e).x;
	y.innerHTML='y='+getMousePosition(e).y;	
	});  
